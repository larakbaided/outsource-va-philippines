...
/**
 * Header for the careers subdomain. Same shell and brand mark as the main
 * site's Header, with careers navigation and an "Outsource VA Philippines"
...
export function CareersHeader() {
  ...
  // Nav hrefs are subdomain-relative ("/jobs"). Server-side the pathname is the
  // rewritten "/careers/jobs"; after hydration it's the visible "/jobs". Strip
  // the prefix so both agree and highlighting doesn't flicker.
  const current = pathname.replace(/^\/careers/, "") || "/";
      ...
      <div className="mx-auto flex h-18 max-w-6xl items-center justify-between px-5 sm:px-6 lg:px-8">
        <Link href="/" aria-label="Outsource VA Philippines careers — home">
          <span className="inline-flex items-center gap-2.5">
            <Logo />
            <span className="hidden border-l border-border pl-2.5 font-sans text-sm font-medium uppercase tracking-[0.14em] text-accent-strong sm:inline">
              Careers
            </span>
          </span>
        </Link>
